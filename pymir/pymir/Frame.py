...
class Frame(numpy.ndarray):
    ...
    def frames(self,fs,winlen=0.025,winhop=0.01,window=None):
        """
        Decompose this frame into smaller overlapping frames of size winlen
        """
        slen = len(self)
        #print slen
        ...
        padsignal = numpy.concatenate((self,zeros))

        indices = numpy.tile(numpy.arange(0,winlen),(numframes,1)) + numpy.tile(numpy.arange(0,numframes*winhop,winhop),(winlen,1)).T
        indices = numpy.array(indices,dtype=numpy.int32)
        frames = padsignal[indices]
        win = numpy.tile(window(winlen),(numframes,1))

        myframes = frames*win
        myframes = myframes.view(Frame)
        return myframes
